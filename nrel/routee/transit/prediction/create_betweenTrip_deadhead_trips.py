import pandas as pd
def create_betweenTrip_deadhead_trips(trips_df: pd.DataFrame, stop_times_df: pd.DataFrame) -> pd.DataFrame:
    ...
    deadhead_trips = pd.DataFrame({'trip_id': [], 'route_id': [], 'service_id': [], 'block_id': [], 
                                   'shape_id': [], 'route_short_name': [], 'route_type': [], 'route_desc': [],
                                   'agency_id': []})
    trip_start = stop_times_df.groupby('trip_id')['arrival_time'].min().reset_index() # trip start time of each trip
    trips_df = trips_df.merge(trip_start, on='trip_id', how='left') # only look at trips on selected date and route
    ...
    for _, block_df in block_gb:
        ...
        block_df = block_df[['deadhead_trip', 'route_id', 'service_id', 'block_id', 'shape_id']]
